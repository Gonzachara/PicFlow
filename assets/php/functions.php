    ...
    $db = mysqli_connect(DB_HOST,DB_USER,DB_PASS,DB_NAME) or die("La base de datos no está conectada");
    ...
    function unblockUser($user_id){
        ...
        createNotification($current_user,$user_id,"te desbloquearon");
        return mysqli_query($db,$query);   
    }
    ...
    if($poster_id!=$current_user){
        createNotification($current_user,$poster_id,"le dio me gusta a tu post!",$post_id);
    }
    ...
    $comment = mysqli_real_escape_string($db,$comment);
        ...
        if($poster_id!=$current_user){
            createNotification($current_user,$poster_id,"comentó en tu posteo",$post_id);
        }
    ...
    $response['status']=true;
        ...
        if(!$form_data['password']){
            $response['msg']="La contraseña es requerida";
            $response['status']=false;
            $response['field']='password';
        }
    
        if(!$form_data['username']){
            $response['msg']="El nombre de usuario es requerido";
            $response['status']=false;
            $response['field']='username';
        }
        
        if(!$form_data['email']){
            $response['msg']="El correo electrónico es requerido";
            $response['status']=false;
            $response['field']='email';
        }
        
        if(!$form_data['last_name']){
            $response['msg']="El apellido es requerido";
            $response['status']=false;
            $response['field']='last_name';
        }
        if(!$form_data['first_name']){
            $response['msg']="El nombre es requerido";
            $response['status']=false;
    ...
    function validateLoginForm($form_data){
        ...
        $blank=false;
            ...
            if(!$form_data['username_email']){
                $response['msg']="El nombre de usuario/correo electrónico es requerido";
                $response['status']=false;
            ...
            if(!$blank && !checkUser($form_data)['status'] ){
                $response['msg']="Algo está incorrecto, no podemos encontrarte";
                $response['status']=false;
    ...
    function validateUpdateForm($form_data,$image_data){
        ...
        if($image_data['name']){
            ...
            if($type!='jpg' && $type!='jpeg' && $type!='png'){
                $response['msg']="Solo se permiten imágenes jpg,jpeg,png";
                $response['status']=false;
                $response['field']='profile_pic';
            }

            if($size>1000){
                $response['msg']="Subir una imagen de menos de 1 MB";
                $response['status']=false;
                $response['field']='profile_pic';
            }
        }
    ...
    function validatePostImage($image_data){
        ...
        $response['status']=true;
            ...
            if(!$image_data['name']){
                $response['msg']="No se ha seleccionado ninguna imagen";
                $response['status']=false;
